...
bool g_bPlayerKillTankrock[MAXPLAYERS + 1];
...
#define PLUGIN_DESCRIPTION	"鱼猫猫领域力场"
#define PLUGIN_LINK			""
...
{
	RegAdminCmd("sm_ymzb", Command_KillMenu, ADMFLAG_ROOT, "领域力场菜单");
	RegAdminCmd("sm_DFF", Command_KillMenu, ADMFLAG_ROOT, "领域力场菜单");
	RegAdminCmd("sm_lylc", Command_KillMenu, ADMFLAG_ROOT, "领域力场菜单");
	RegAdminCmd("sm_jbgod", Command_KillMenu, ADMFLAG_ROOT, "领域力场菜单");

	g_cvarAutoEnableOnMenu = CreateConVar(PLUGIN_NAME ... "_auto_enable",	"1", 	"打开菜单时自动开启力场 (0=关闭, 1=开启)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarKillCommon = CreateConVar(PLUGIN_NAME ... "_common",				"1", 	"是否秒杀小僵尸 (0=关闭, 1=开启)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarKillSpecial = CreateConVar(PLUGIN_NAME ... "_special",			"1", 	"是否秒杀特感 (0=关闭, 1=开启)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarKillTank = CreateConVar(PLUGIN_NAME ... "_tank",					"0", 	"是否秒杀坦克 (0=关闭, 1=开启)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarKillTankrock = CreateConVar(PLUGIN_NAME ... "_tank_rock",			"1", 	"是否清除坦克投掷 (0=关闭, 1=开启)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarKillWitch = CreateConVar(PLUGIN_NAME ... "_witch",				"0", 	"是否秒杀女巫 (0=关闭, 1=开启)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_cvarKillRange = CreateConVar(PLUGIN_NAME ... "_range",				"256",	"力场范围", FCVAR_NONE, true, 50.0, true, 1000.0);
	...
	g_cvarKillTankrock.AddChangeHook(OnConVarChanged);
...
void GetCVarValues() 
{
	...
	g_bKillTankrock = g_cvarKillTankrock.BoolValue;
}
...
public void OnClientPutInServer(int client) 
{
	...
	if (!playerLoad[client])
	{
		...
		g_bPlayerKillTankrock[client] = g_bKillTankrock;
		OnGmKill[client] = 0;
...
public void Event_PlayerDisconnect(Event event, const char[] name, bool dontBroadcast)
{
	...
	g_bPlayerKillTankrock[client] = false;
...
public Action Command_KillMenu(int client, int args)
{
	...
	if (g_bAutoEnableOnMenu && OnGmKill[client] == 0)
	{
		...
		//PrintToChat(client, "\x04%s\x05领域力场已\x03开启\x05喵！", PREFIX);
	}
...
void ShowMainMenu(int client)
{
	...
	menu.SetTitle("鱼猫猫领域力场\n当前状态：%s", 
		OnGmKill[client] == 1 ? "开启" : "关闭");
	
	Format(buffer, sizeof(buffer), "%s力场", OnGmKill[client] == 1 ? "关闭" : "开启");
...
void ToggleKillFunction(int client)
{
	...
	if (OnGmKill[client] == 1) 
	{
		...
		PrintToChat(client, "\x04%s\x05领域力场已 \x03关闭 \x05喵！", PREFIX);
	} 
	else 
	{
		OnGmKill[client] = 1;
		...
		PrintToChat(client, "\x04%s\x05坦克投掷: \x03%s", PREFIX, g_bPlayerKillTankrock[client] ? "是" : "否");
		PrintToChat(client, "\x04%s\x05秒杀女巫: \x03%s", PREFIX, g_bPlayerKillWitch[client] ? "是" : "否");
		PrintToChat(client, "\x04%s\x05力场范围: \x03%.1f", PREFIX, g_fPlayerKillRange[client]);
...
void ShowSettingsMenu(int client)
{
	...
	Format(buffer, sizeof(buffer), "坦克投掷: %s", g_bPlayerKillTankrock[client] ? "是" : "否");
	menu.AddItem("Tankrock", buffer);
	
	Format(buffer, sizeof(buffer), "秒杀女巫: %s", g_bPlayerKillWitch[client] ? "是" : "否");
	menu.AddItem("witch", buffer);
	
	Format(buffer, sizeof(buffer), "力场范围: %.0f", g_fPlayerKillRange[client]);
...
public int SettingsMenuHandler(Menu menu, MenuAction action, int client, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		else if (StrEqual(info, "Tankrock"))
		{
			g_bPlayerKillTankrock[client] = !g_bPlayerKillTankrock[client];
			PrintToChat(client, "\x04%s\x05 坦克投掷: \x03%s", PREFIX, 
				g_bPlayerKillTankrock[client] ? "开启" : "关闭");
			ShowSettingsMenu(client);
		}
...
public int RangeMenuHandler(Menu menu, MenuAction action, int client, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		PrintToChat(client, "\x04%s\x05 力场范围已设置为: \x03%.0f", PREFIX, range);
		ShowSettingsMenu(client);
	}
	else if (action == MenuAction_Cancel && param2 == MenuCancel_ExitBack)
	{
...
{
	PrintToChat(client, "\x04%s\x05 ======= \x04当前力场设置\x05 =======", PREFIX);
	...
	PrintToChat(client, "\x04%s\x05秒杀坦克: \x03%s", PREFIX, 
		g_bPlayerKillTank[client] ? "开启" : "关闭");
	PrintToChat(client, "\x04%s\x05坦克投掷: \x03%s", PREFIX,
		g_bPlayerKillTankrock[client] ? "开启" : "关闭");
	PrintToChat(client, "\x04%s\x05秒杀女巫: \x03%s", PREFIX, 
		g_bPlayerKillWitch[client] ? "开启" : "关闭");
	PrintToChat(client, "\x04%s\x05力场范围: \x03%.0f", PREFIX, 
...
public Action Timer_KillZombies(Handle timer, any client) 
{
	...
	bool useTankrock = g_bPlayerKillTankrock[client];
