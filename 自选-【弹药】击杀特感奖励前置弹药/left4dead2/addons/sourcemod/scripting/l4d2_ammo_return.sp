...
int GetAmmoRewardByWeapon(int weapon, int headshot);
...
public void OnPluginStart()
{
	...
	hTankMultiplier 			= CreateConVar("l4d2_ammo_tank_multiplier", 			"4", 	"击杀坦克奖励为当前武器爆头击杀奖励值的倍数.", FCVAR_NOTIFY);
	hWitchMultiplier 			= CreateConVar("l4d2_ammo_witch_multiplier", 			"3", 	"击杀女巫奖励为当前武器爆头击杀奖励值的倍数.", FCVAR_NOTIFY);
	hWitchOneShotMultiplier		= CreateConVar("l4d2_ammo_witch_oneshot_multiplier", 	"5", 	"秒杀女巫奖励为当前武器爆头击杀奖励值的倍数.", FCVAR_NOTIFY);
	...
	hTankMultiplier.AddChangeHook(ConVarChanged);
	hWitchMultiplier.AddChangeHook(ConVarChanged);
	hWitchOneShotMultiplier.AddChangeHook(ConVarChanged);
...
void LoadConVarValues()
{
	...
	TankMultiplier = hTankMultiplier.IntValue;
	WitchMultiplier = hWitchMultiplier.IntValue;
	WitchOneShotMultiplier = hWitchOneShotMultiplier.IntValue;
}
...
int GetAmmoRewardByWeapon(int weapon, int headshot)
{
	...
	if(StrEqual(weaponname, "sniper_awp", false)) // Minigun
	{
		return headshot == 0 ? CountMinigunAmmoHead : CountMinigunAmmoShot;
	}
	else if(StrEqual(weaponname, "sniper_scout", false)) // 固定机枪
	{
		return headshot == 0 ? CountTurretAmmoHead : CountTurretAmmoShot;
	}
	else
	{
		int PrimType = GetEntProp(weapon, Prop_Send, "m_iPrimaryAmmoType");
		switch(PrimType)
		{
			case 1: return headshot == 0 ? CountPistolAmmoHead : CountPistolAmmoShot;
			case 2: return headshot == 0 ? CountMagnumAmmoHead : CountMagnumAmmoShot;
			case 3: return headshot == 0 ? CountRifleAmmoHead : CountRifleAmmoShot;
			case 5: return headshot == 0 ? CountSmgAmmoHead : CountSmgAmmoShot;
			case 6: return headshot == 0 ? CountM60AmmoHead : CountM60AmmoShot;
			case 7: return headshot == 0 ? CountShotgunAmmoHead : CountShotgunAmmoShot;
			case 8: return headshot == 0 ? CountAutoshotAmmoHead : CountAutoshotAmmoShot;
			case 9: return headshot == 0 ? CountHuntingAmmoHead : CountHuntingAmmoShot;
			case 10: return headshot == 0 ? CountSniperAmmoHead : CountSniperAmmoShot;
			case 17: return headshot == 0 ? CountGrenadeAmmoHead : CountGrenadeAmmoShot;
			case 19: return headshot == 0 ? CountChainsawHead : CountChainsawShot;
		}
	}
	return 0;
}
...
public void Event_TankKilled(Event event, const char[] name, bool dontBroadcast)
{
	...
	int baseReward = GetAmmoRewardByWeapon(Weapon, 1);
	int ammoReward = 0;
	
	if(baseReward > 0)
	{
		ammoReward = baseReward * TankMultiplier;
	}
	...
	if(ammoReward > 0)
	{
		...
		{
			GetTrueName(attacker, clientName);
			PrintToChatAll("\x04%s\x03%s\x05击杀\x03坦克\x04,\x05奖励\x03%d\x05前置弹药(基础爆头%d乘以%d).", PREFIX, clientName, ammoReward, baseReward, TankMultiplier);
		}
	}
}
...
public void Event_WitchKilled(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(baseReward > 0)
	{
		if(oneShot)
		{
			ammoReward = baseReward * WitchOneShotMultiplier;
		}
		else
		{
			ammoReward = baseReward * WitchMultiplier;
		}
	}
	...
	if(ammoReward > 0)
	{
		...
		if(bShowKillMessages)
		{
			...
			if(oneShot)
				PrintToChatAll("\x04%s\x03%s\x05秒杀\x03女巫\x04,\x05奖励\x03%d\x05前置弹药(基础爆头%d乘以%d).", PREFIX, clientName, ammoReward, baseReward, WitchOneShotMultiplier);
			else
				PrintToChatAll("\x04%s\x03%s\x05击杀\x03女巫\x04,\x05奖励\x03%d\x05前置弹药(基础爆头%d乘以%d).", PREFIX, clientName, ammoReward, baseReward, WitchMultiplier);
		}
	}
}
